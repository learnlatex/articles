...
\usepackage{csquotes}
\usepackage{hologo}
%\usepackage{xurl} % roman underscores?
...
\netaddress{d dot p dot carlisle (at) gmail.com}
\author{Paulo Roberto Massa Cereda}
\address{S\~{a}o Paulo, Brazil}
\netaddress{paulocereda (at) gmail.com}
...
by Nicola Talbot and used by the UK \TeX{} User Group (\acro{UK-TUG}) for
...
\hologo{pdfLaTeX}, not other engines such as \hologo{LuaLaTeX}. So the decision was
made to use a server which ran \LaTeX{}, returning the generated \acro{PDF}
...
notably p\LaTeX{}, \hologo{LuaLaTeX} and \hologo{XeLaTeX} in addition to
\hologo{pdfLaTeX}.
...
The most noticeable distinguishing feature of the \LaTeX{}~\acro{CGI} server
is that it does not directly return the generated \acro{PDF} but instead
returns a
...
mobile browsers which typically do not include a built-in \acro{PDF}
renderer.
The server does support options to return the \acro{PDF} directly, or to return
the log file, even of successful runs.

\LaTeX{}~\acro{CGI} supports a range of \LaTeX{} formats:
...
Amazon's \acro{EC2} service, which is available in the \emph{Free Tier} for one
...
The Overleaf \acro{API} was extended for this site, to allow multiple-file
projects to be uploaded in this way.

Other \TeX{} editing systems could be used but a requirement is a
public \acro{API} that starts a project via an \acro{HTTP} request rather than using
